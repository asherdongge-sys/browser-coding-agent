...
export class WorkspaceManager {
  ...
  getRoot(): string {
    if (!this.root) throw new Error("No workspace selected");
    return this.root;
  }
...
export function createFilesystemTools(workspace: WorkspaceManager): Tool[] {
  const list: Tool<FsListArguments, string[]> = {
    ...
    async execute(args) {
      try {
        const entries = await fs.readdir(workspace.resolve(args.path ?? "."), { withFileTypes: true });
        return { ok: true, result: entries.map((entry) => `${entry.isDirectory() ? "dir" : "file"}\t${entry.name}`) };
      } catch (error) {
  ...
  const read: Tool<FsReadArguments, string> = {
    ...
    async execute(args) {
      try {
        const content = await fs.readFile(workspace.resolve(args.path), args.encoding ?? "utf8");
        return { ok: true, result: content.toString() };
      } catch (error) {
        return { ok: false, error: error instanceof Error ? error.message : String(error) };
      }
    },
  ...
  const search: Tool<FsSearchArguments, string[]> = {
    ...
    async execute(args) {
      try {
        const root = workspace.resolve(args.path ?? ".");
        ...
        async function walk(directory: string): Promise<void> {
          for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
            ...
            else if (entry.isFile() && (await fs.stat(target)).size < 1024 * 1024) {
              const content = await fs.readFile(target, "utf8").catch(() => "");
              if (content.includes(args.query)) results.push(path.relative(workspace.getRoot(), target));
            }
          }
        }
        await walk(root);
