...
async function call(name: string, args: any): Promise<any> {
  ...
  if (name.startsWith("github.") && github) return github.callTool(name, args);
  throw new Error(`Unknown MCP tool: ${name}`);
}

const tools = () => [...browserTools, ...githubTools];
...
rl.on("line", async (line) => {
  ...
  try {
    if (request.method === "initialize") {
      process.stdout.write(JSON.stringify({ jsonrpc: "2.0", id: request.id, result: { protocolVersion: "2025-06-18", capabilities: { tools: {} }, serverInfo: { name: "browser-coding-agent-runtime", version: "0.1.0" } } }) + "\n");
      return;
    }
    if (request.method === "tools/list") { process.stdout.write(JSON.stringify({ jsonrpc: "2.0", id: request.id, result: { tools: tools() } }) + "\n"); return; }
    if (request.method === "tools/call") {
      const result = await call(request.params?.name, request.params?.arguments ?? {});
      process.stdout.write(JSON.stringify({ jsonrpc: "2.0", id: request.id, result: { structuredContent: result, content: [{ type: "text", text: JSON.stringify(result) }] } }) + "\n");
      return;
    }
    process.stdout.write(JSON.stringify({ jsonrpc: "2.0", id: request.id, error: { code: -32601, message: `Method not found: ${request.method}` } }) + "\n");
  } catch (error) {
    process.stdout.write(JSON.stringify({ jsonrpc: "2.0", id: request.id, result: { isError: true, content: [{ type: "text", text: error instanceof Error ? error.message : String(error) }] } }) + "\n");
  }
});
