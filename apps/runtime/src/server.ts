...
export function createRuntimeServer(port = Number(process.env.BROWSER_CODING_AGENT_PORT ?? DEFAULT_PORT)) {
  ...
  const safeBroadcast = (message: unknown, role?: Role): void => {
    for (const client of clients) if (!role || roles.get(client) === role) safeSend(client, message);
  };
  ...
  const ensureGitHubMcp = async (): Promise<McpStdioClient> => {
    ...
    githubMcpStartup = (async () => {
      const client = new McpStdioClient(process.execPath, [GITHUB_MCP_SERVER], { GITHUB_ACCESS_TOKEN: connection.accessToken });
      await client.start();
  ...
  const executeTool = <TArguments = unknown, TResult = unknown>(call: ToolCall<TArguments>, requester?: WebSocket): Promise<ToolResult<TResult>> => {
    ...
    if (decision === "ask") {
      return new Promise<ToolResult<TResult>>((resolveResult) => {
        ...
        pending.set(requestId, { socket: requester, call, toolName: tool.descriptor.name, resolve: (result) => resolveResult(result as ToolResult<TResult>) });
        safeBroadcast({ jsonrpc: "2.0", method: "approval.request", params: { requestId, tool: tool.descriptor.name, risk: tool.descriptor.risk, description: tool.descriptor.description, arguments: call.arguments }});
      });
    }
  ...
  wsServer.on("connection", (socket) => {
    ...
    socket.on("message", async (raw: Buffer) => {
      ...
      try {
        ...
        if (message.method === "agent.create") { const provider = await ensureBrowser(); const agent = await provider.createAgent(typeof params.title === "string" ? params.title : "", typeof params.prompt === "string" ? params.prompt : ""); reply(socket, id, { agent }); return; }
        if (message.method === "agent.send") {
          ...
          // All user chat goes through the ChatGPT tab. GitHub questions are handled
          // by the provider's @GitHub App selection rather than the local GitHub MCP.
