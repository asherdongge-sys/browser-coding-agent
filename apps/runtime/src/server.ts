...
export function createRuntimeServer(port = Number(process.env.BROWSER_CODING_AGENT_PORT ?? DEFAULT_PORT)) {
  ...
  const safeBroadcast = (message: unknown, role?: Role): void => {
    ...
    for (const client of clients) {
      if (!role || roles.get(client) === role) safeSend(client, payload);
    }
  };
  ...
  const sendJson = (response: import("node:http").ServerResponse, status: number, body: unknown): void => {
  ...
  const httpServer = createServer(async (request, response) => {
    if (shuttingDown) {
      sendJson(response, 503, { ok: false, error: "Runtime is shutting down" });
      return;
    }
    ...
    if (requestUrl.pathname === "/api/github/connect") {
      try {
        response.writeHead(302, {
          location: await createGitHubAuthorizeUrl(githubOAuthCallbackUrl(port)),
          "cache-control": "no-store",
        });
        response.end();
      } catch (error) {
    ...
    sendJson(response, 200, {
      ...
      dashboards: [...roles.values()].filter((role) => role === "dashboard").length,
      extensions: [...roles.values()].filter((role) => role === "extension").length,
  ...
  wsServer.on("connection", (socket) => {
    ...
    socket.on("message", async (raw: Buffer) => {
      ...
      try {
        ...
        safeSend(socket, { jsonrpc: "2.0", id: "invalid", error: { code: -32700, message: "Invalid JSON" } });
        return;
      }
      ...
      try {
        ...
        if (message.method === "runtime.hello") {
          ...
          if (role === "dashboard") {
            const provider = providerKind === "playwright" ? await ensureBrowser() : undefined;
            reply(socket, id, {
              ok: true,
              protocol: "0.1",
              planner: "chatgpt-browser",
              browserProvider: provider?.kind ?? providerKind,
              browserStartupError: browserStartupError?.message,
            });
            if (provider) {
              const agents = await provider.listAgents();
              for (const agent of agents) safeSend(socket, { jsonrpc: "2.0", method: "dashboard.event", params: { type: "agent.updated", agent } });
            }
          } else {
        ...
        if (message.method === "runtime.ping") {
          reply(socket, id, { ok: true, protocol: "0.1", planner: "chatgpt-browser", browserProvider: providerKind });
          return;
        }

        if (message.method === "agent.list") {
          const provider = await ensureBrowser();
          reply(socket, id, { agents: await provider.listAgents() });
          return;
        }
        ...
        if (message.method === "agent.send") {
          ...
          void provider.sendMessage(agentId, text).catch((error) => {
            safeBroadcast({
              ...
              params: {
                type: "agent.updated",
                agent: { id: agentId, status: "failed", lastError: error instanceof Error ? error.message : String(error), updatedAt: Date.now() },
              },
            }, "dashboard");
          });
        ...
        if (message.method === "workspace.select") {
          reply(socket, id, { root: workspace.select(requiredString(params.path, "path")) });
          return;
        }
        if (message.method === "workspace.info") {
          reply(socket, id, { root: safeWorkspaceRoot(workspace) });
          return;
        }
        if (message.method === "tools.list") {
          reply(socket, id, tools.list());
          return;
        }
