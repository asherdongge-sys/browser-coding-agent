...
import { ToolRegistry, WorkspaceManager, createFilesystemTools } from "@browser-coding-agent/tools";
...
export function createRuntimeServer(port = Number(process.env.BROWSER_CODING_AGENT_PORT ?? DEFAULT_PORT)) {
  ...
  for (const tool of createFilesystemTools(workspace)) tools.register(tool);
  ...
  wsServer.on("connection", (socket) => {
    socket.on("message", async (raw: Buffer) => {
      ...
      try {
        ...
        socket.send(JSON.stringify({ jsonrpc: "2.0", id: "invalid", error: { code: -32700, message: "Invalid JSON" } }));
        return;
      }
      ...
      try {
        if (message.method === "runtime.ping") {
          reply(socket, id, { ok: true, protocol: "0.1" });
          return;
        }
        ...
        if (message.method === "tool.call") {
          ...
          if (decision !== "allow") {
            reply(socket, id, { ok: false, error: `Permission ${decision} for ${tool.descriptor.name}` });
            return;
          }
          reply(socket, id, await tool.execute(call.arguments));
          return;
        }
...
function asRecord(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object") throw new Error("params must be an object");
...
function reply(socket: import("ws").WebSocket, id: string | number, result?: unknown, error?: { code: number; message: string }): void {
  socket.send(JSON.stringify({ jsonrpc: "2.0", id, ...(error ? { error } : { result }) }));
}
