...
export function createRuntimeServer(port = Number(process.env.BROWSER_CODING_AGENT_PORT ?? DEFAULT_PORT)) {
  ...
  const pending = new Map<string, { socket: WebSocket; id: string | number; call: ToolCall; toolName: string }>();
  ...
  wsServer.on("connection", (socket) => {
    socket.on("message", async (raw: Buffer) => {
      ...
      try { message = JSON.parse(raw.toString()) as RpcMessage; } catch { socket.send(JSON.stringify({ jsonrpc: "2.0", id: "invalid", error: { code: -32700, message: "Invalid JSON" } })); return; }
      ...
      try {
        ...
        if (message.method === "tool.call") {
          ...
          if (decision === "ask") {
            ...
            socket.send(JSON.stringify({ jsonrpc: "2.0", method: "approval.request", params: { requestId, tool: tool.descriptor.name, risk: tool.descriptor.risk, description: tool.descriptor.description, arguments: call.arguments } })); return;
          }
    ...
    socket.on("close", () => { for (const [requestId, request] of pending) if (request.socket === socket) pending.delete(requestId); });
  });
...
function reply(socket: WebSocket, id: string | number, result?: unknown, error?: { code: number; message: string }): void { socket.send(JSON.stringify({ jsonrpc: "2.0", id, ...(error ? { error } : { result }) })); }
