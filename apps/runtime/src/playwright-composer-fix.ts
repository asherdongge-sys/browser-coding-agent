...
const selectionLocks = new WeakMap<Page, Promise<boolean>>();
...
async function composerContainsMention(page: Page, appName: string): Promise<boolean> {
  const composer = await findComposer(page);
  if (!composer) return false;
  const value = await composer.evaluate((node) => {
...
provider.selectChatGPTApp = async function selectChatGPTApp(page: Page, appName: string): Promise<boolean> {
  ...
  const existing = selectionLocks.get(page);
  if (existing) return existing;
  const operation = (async () => {
    if (await composerContainsMention(page, appName)) return ensureGitHubSelected(page, appName);
    return ensureGitHubSelected(page, appName);
  })();
  selectionLocks.set(page, operation);
  try { return await operation; }
  finally { if (selectionLocks.get(page) === operation) selectionLocks.delete(page); }
};
...
provider.createAgent = async function createAgent(title: string, prompt: string): Promise<unknown> {
  ...
  const self = this;
  const emit = (event: unknown) => (self as unknown as { onEvent?: (event: unknown) => void }).onEvent?.(event);
  const patch = (changes: Record<string, unknown>) => {
    const managed = self.agents?.get(agent.id);
    if (managed) Object.assign(managed, changes, { updatedAt: Date.now() });
  };
  await startChatGPTPageSync(agent as never, agent.page, emit as never, patch as never).catch(() => undefined);
  return agent;
};
...
provider.submitComposer = async function submitComposer(page: Page, text: string): Promise<void> {
  ...
  const sendSelectors = ["button[data-testid='send-button']", "button[aria-label*='Send' i]", "button[aria-label*='发送' i]", "button[type='submit']"];
