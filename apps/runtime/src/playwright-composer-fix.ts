...
provider.initializeAgent = async function initializeAgent(agent: Managed): Promise<void> {
  try {
    ...
    if (agent.prompt) {
      ...
      delete agent.prompt;
      // Selection is already committed. Reserve the native send check and the
      // runtime's post-create initialization check without touching the UI twice.
...
provider.selectChatGPTApp = async function selectChatGPTApp(page: Page, appName: string): Promise<boolean> {
  ...
  return ensureGitHubSelectedV2(page, appName).then((selected) => {
    const agent = agentForPage(this, page);
    if (agent) patchAgent(this, agent.id, selected
