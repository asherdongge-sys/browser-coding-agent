...
type ManagedAgent = BrowserAgent & { page: Page; initialPrompt?: string };
...
export class PlaywrightBrowserProvider implements BrowserProvider {
  ...
  async listAgents(): Promise<BrowserAgent[]> {
    return [...this.agents.values()].map(({ page: _page, initialPrompt: _prompt, ...agent }) => agent);
  }
  ...
  async createAgent(title: string, prompt: string): Promise<BrowserAgent> {
    ...
    void initialization.finally(() => {
      if (this.initialization.get(agent.id) === initialization) this.initialization.delete(agent.id);
    });
  ...
  private async keepDashboardForeground(): Promise<void> {
    ...
    if (!page || page.isClosed() || this.headless || this.stopping) return;
  ...
  private async findComposer(page: Page): Promise<{ kind: "contenteditable" | "textarea" | "role"; index: number } | undefined> {
    try {
      return await page.evaluate(() => {
        const groups = [["[contenteditable='true']", "contenteditable"], ["textarea", "textarea"], ["[role='textbox']", "role"]] as const;
        for (const [selector, kind] of groups) {
          ...
          for (let index = nodes.length - 1; index >= 0; index -= 1) {
            ...
            const style = getComputedStyle(node);
            const rect = node.getBoundingClientRect();
            if (style.display !== "none" && style.visibility !== "hidden" && rect.width > 20 && rect.height > 10) return { kind, index };
          }
        }
        return undefined;
      });
  ...
  private async latestAssistant(page: Page): Promise<Snapshot> {
  ...
  private async latestUser(page: Page): Promise<Snapshot> {
    return page.evaluate(() => {
  ...
  private async waitForUserTurn(page: Page, previous: Snapshot, expected: string): Promise<void> {
  ...
  private async waitForAssistant(page: Page, previous: Snapshot, agent: ManagedAgent): Promise<string> {
  ...
  private ensureMessages(agent: ManagedAgent): BrowserAgentMessage[] {
  ...
  private updateStreamingAssistant(agent: ManagedAgent, text: string): void {
    ...
    else messages.push({ role: "assistant", text, createdAt: Date.now() });
  }
