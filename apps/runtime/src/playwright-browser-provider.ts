...
export class PlaywrightBrowserProvider implements BrowserProvider {
  ...
  private async waitForAssistant(page: Page, previous: AssistantSnapshot, agent: ManagedAgent): Promise<string> {
    const deadline = Date.now() + RESPONSE_TIMEOUT_MS; let last = ""; let stableSince = 0; let streamed = "";
    while (Date.now() < deadline) { const current = await this.latestAssistant(page); const changed = current.count > previous.count || (current.text.length > 0 && current.text !== previous.text); if (changed && current.text) { if (current.text !== streamed) { streamed = current.text; this.updateStreamingAssistant(agent, streamed); this.emit({ type: "agent.message", agentId: agent.id, role: "assistant", text: streamed, url: agent.page.url(), createdAt: Date.now(), streaming: true }); } if (current.text !== last) { last = current.text; stableSince = Date.now(); } if (Date.now() - stableSince >= STABILITY_MS) return current.text; } await page.waitForTimeout(400); }
