...
async function saveAgents(agents: AgentRecord[]): Promise<void> {
...
async function createAgent(title: string, prompt: string): Promise<AgentRecord> {
  ...
  try {
    ...
    await updateAgent(agent.id, { status: "idle", conversationUrl: (await chrome.tabs.get(tab.id)).url });
...
async function sendAgentMessage(agentId: string, text: string): Promise<void> {
  ...
  await updateAgent(agentId, { status: "waiting", lastError: undefined, conversationUrl: (await chrome.tabs.get(agent.tabId)).url });
}
...
chrome.runtime.onMessage.addListener((message: unknown, sender, sendResponse) => {
  ...
  if (request.type === "agent.list") {
    void getAgents().then((agents) => sendResponse({ ok: true, agents }));
    return true;
  }
  ...
  if (request.type === "agent.resume") {
    ...
    void (async () => {
      ...
      await updateAgent(agentId, { status: "idle", lastError: undefined, conversationUrl: (await chrome.tabs.get(agent.tabId)).url });
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo) => {
  ...
  void getAgents().then(async (agents) => {
    ...
    if (!agent) return;
    await updateAgent(agent.id, { conversationUrl: changeInfo.url ?? agent.conversationUrl });
  });
});
...
chrome.tabs.onRemoved.addListener((tabId) => {
  void getAgents().then(async (agents) => {
    const agent = agents.find((item) => item.tabId === tabId);
    if (agent) await updateAgent(agent.id, { tabId: undefined, status: "tab-closed" });
  });
});
