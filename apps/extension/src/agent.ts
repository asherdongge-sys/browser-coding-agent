...
type AgentEvent = {
  type?: "agent.state" | "agent.message";
  agentId?: string;
  state?: string;
  role?: "user" | "assistant";
  text?: string;
...
function renderAgents(): void {
  agentsEl.innerHTML = "";
  if (!agents.length) {
    const empty = document.createElement("div"); empty.className = "muted"; empty.textContent = "还没有 Agent"; agentsEl.append(empty); return;
  }
  for (const agent of agents) {
    const button = document.createElement("button");
    button.className = `agent-item${agent.id === selectedAgentId ? " active" : ""}`;
...
function renderMessages(): void {
  ...
  for (const message of agentMessages) {
    const item = document.createElement("div"); item.className = `message ${message.role}`; item.textContent = message.text; messagesEl.append(item);
  }
...
function selectAgent(id: string): void {
  ...
  inputEl.disabled = agent.status === "opening-chatgpt";
...
function appendMessage(agentId: string, role: "user" | "assistant", text: string): void {
  ...
  list.push({ role, text, createdAt: Date.now() });
...
chrome.runtime.onMessage.addListener((message: { type?: string; agent?: AgentRecord; params?: AgentEvent }) => {
  ...
  if (message.type === "agent.updated" && message.agent) {
    ...
    if (selectedAgentId === message.agent.id) selectAgent(message.agent.id); else renderAgents();
    return;
  }
  if (message.type !== "agent.event" || !message.params?.agentId) return;
  const event = message.params;
  if (event.type === "agent.message" && event.role && event.text) appendMessage(event.agentId, event.role, event.text);
  if (event.type === "agent.state") {
    const agent = agents.find((item) => item.id === event.agentId);
    if (agent) {
      agent.status = event.state ?? agent.status;
      agent.conversationUrl = agent.conversationUrl || undefined;
      renderAgents();
      if (selectedAgentId === agent.id) selectAgent(agent.id);
    }
  }
});
...
inputEl.addEventListener("keydown", (event) => {
  if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); void sendCurrentMessage(); }
});
